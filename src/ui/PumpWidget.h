...
private:
    ...
    struct ChannelPanel {
        ...
        QLabel*          warn    = nullptr;   // ⚠ sonde absente (tooltip = cause)
        ...
        QSpinBox*        zeroDuty = nullptr;   // seuil zéro-RPM manuel (0 = auto)
    ...
    void applyFaultStyle(ChannelPanel* p);            // ⚠ + orange si sonde absente
    ...
    // Calibration : pales en rotation pour chaque ventilo calibré, vitesse
    // proportionnelle au duty du palier courant (100 %, 90 %...), avec montée
    // en charge / ralenti progressifs (même dynamique que l'identify).
    struct CalibSpin {
        qreal angle     = 0.0;
        qreal vel       = 0.0;   // °/s courant
        qreal targetVel = 0.0;   // °/s cible (∝ duty)
        bool  stopping  = false; // décélère puis retiré
    };
    QMap<QString, CalibSpin> m_calibSpin;
    void ensureSpinTimer();      // crée le timer + le pilote (identify + calib)
    ...
    QMap<QString, QString> m_faultReason;          // id -> cause sonde absente (⚠ orange)
