...
void PumpWorker::fanTick()
{
    ...
    for (auto it = m_fanCfg.constBegin(); it != m_fanCfg.constEnd(); ++it) {
        ...
        // Plancher de rotation (zero-RPM) : sous ce seuil le ventilo ne tient
        // pas -> OFF. Priorité au seuil manuel (cfg.zeroBelow), sinon seuil
        // mesuré à la calibration, sinon plancher global 5 %.
        const int dutyFloor = (cfg.zeroBelow > 0) ? cfg.zeroBelow
                            : (cfg.startDuty > 0) ? cfg.startDuty : 5;
        if (isPump) {
        ...
        // arrêté ne repart pas de façon fiable à bas duty). Marge de +5 sur le
        // plancher effectif (manuel ou calibré — le démarrage à froid demande
        // plus que le maintien) ; sans plancher connu, 30 % par défaut.
        if (!isPump && last == 0 && target > 0)
            target = std::max(target,
                              (dutyFloor > 5) ? std::min(100, dutyFloor + 5) : 30);
