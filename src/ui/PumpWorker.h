...
struct FanChannelConfig {
    ...
    int             zeroBelow = 0;        // duty sous lequel le ventilo s'arrête
                                          // (0 RPM). 0 = auto (seuil calibré).
...
public slots:
    ...
    void calibrate();
    // Canal masqué par l'utilisateur : cesser de le piloter et le rendre au
    // BIOS (sinon un canal caché en cours de session resterait piloté).
    void releaseChannel(QString id);

public:
    // Headers identifiés comme tach de la pompe Kraken (persistés par le
    // widget) — masqués dès l'énumération, PWM rendu au BIOS, surveillance
    // continue (divergence -> ré-affiché).
...
signals:
    ...
    // Canaux dont la sonde configurée est absente (driver GPU non installé,
    // sonde disparue...) — rendus au BIOS. reasons = cause par canal (tooltip UI).
    void fanSensorFault(const QStringList& ids, const QStringList& reasons);
    ...
    // Ventilos en cours de calibration + duty du palier courant (0/vide = fin).
    // L'UI fait tourner les pales à une vitesse proportionnelle au duty.
    void calibrationSpin(const QStringList& ids, int dutyPercent);
...
private:
    ...
    void publishLhwmSources();   // (re)construit m_srcIds + émet sources/canaux
    void maybeRetryLhwm();       // ré-init LHM différée (driver pas prêt au boot)
    ...
    // Ré-init LHM : si init() aboutit avec 0 canal (driver kernel pas prêt au
    // lancement de session), on relance toutes les m_lhwmRetryEvery s, jusqu'à
    // m_lhwmRetriesLeft tentatives — puis re-publie sources + canaux.
    int           m_lhwmRetriesLeft = 0;
    int           m_lhwmRetryTick   = 0;
    ...
    QMap<QString, int>              m_srcBadStreak;  // ticks consécutifs sonde invalide
    QStringList                     m_lastFaultIds;  // dernier set émis (détection changement)
    QStringList                     m_lastFaultReasons; // causes parallèles (carry-over identify)
    ...
    int                             m_calibSpinStep = -1; // dernier palier signalé à l'UI (spin)
